...
export async function listCustomVocabWords() {
  ...
  if (error) throw error;
  return (data || []).map((r) => ({ id: r.id, w: r.word, m: r.meaning, createdAt: r.created_at }));
}
...
/** Permanently deletes a user-added word (row id from vocab_words). */
export async function deleteCustomVocabWord(id) {
  const supabase = createClient();
  const { error } = await supabase.from("vocab_words").delete().eq("id", id);
  if (error) throw error;
}

/** Words hidden from a base (built-in) list — BASE_WORDS is shared code, so "deleting" one only hides it for this user. */
export async function listHiddenVocabWords() {
  const supabase = createClient();
  const { data, error } = await supabase.from("vocab_hidden_words").select("word");
  if (error) throw error;
  return (data || []).map((r) => r.word);
}

export async function hideVocabWord(userId, word) {
  const supabase = createClient();
  const { error } = await supabase.from("vocab_hidden_words").upsert({ user_id: userId, word }, { onConflict: "user_id,word" });
  if (error) throw error;
}

/** Clears spaced-repetition progress for a word, e.g. after deleting it. */
export async function deleteVocabProgress(word) {
  const supabase = createClient();
  const { error } = await supabase.from("vocab_progress").delete().eq("word", word);
  if (error) throw error;
}
