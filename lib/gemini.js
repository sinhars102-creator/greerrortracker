...
// scales with prompt/reasoning complexity (a few hundred tokens for a short
// grading call, 3000+ for something like /api/classify's multi-step
// root-cause comparison against up to 60 prior entries). It can't be fully
// disabled either (thinkingBudget: 0 is rejected as invalid on at least
// gemini-flash-latest). So the ceiling we send has to comfortably cover
// thinking *and* the answer — padding it costs nothing extra since Gemini
// bills actual tokens used, not the ceiling (model cap is 65536), and being
// stingy here is exactly what causes real answers to get cut off by their
// own reasoning.
