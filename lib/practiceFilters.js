...
// one of these four (a strict partition).
//   1. mistakes        — ever gotten wrong (worst offenders first). Permanent
//                         membership: wrongAttempts only ever increments.
//                         Takes priority over "recent" — a mistake should
//                         surface immediately, not get hidden in Recent
//                         until it ages out of the 3-day window.
//   2. recent          — logged today or in the last RECENT_DAYS days
//                         (and never gotten wrong), newest first.
...
//
// "starredMistakes" is a further narrowing of "starred": just the starred
// entries gotten wrong SINCE being starred (not ones that were already
// wrong before you starred them) — the ones worth the most drilling.
// wrongAttemptsAtStar is a snapshot taken the moment starring last turned
// on (see QuestionCard's star toggle), so "since starring" = wrongAttempts
// has grown past that snapshot. Also cross-cutting/additive, same as
// "starred".
export function buildTiers(entries) {
  ...
  const starredMistakes = starred
    .filter((e) => (e.wrongAttempts || 0) > (e.wrongAttemptsAtStar ?? 0))
    .sort((a, b) => ((b.wrongAttempts || 0) - (b.wrongAttemptsAtStar ?? 0)) - ((a.wrongAttempts || 0) - (a.wrongAttemptsAtStar ?? 0)));

  return { recent, mistakes, neverAttempted, rest, starred, starredMistakes };
}

export function flattenTiers(tiers) {
  return [...tiers.mistakes, ...tiers.recent, ...tiers.neverAttempted, ...tiers.rest];
}
