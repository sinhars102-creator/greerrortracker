...
// every time. "Not reviewed today yet" is sorted ahead of "already
// reviewed today" within each bucket (worst-offenders/newest-logged still
// break ties inside each half) so finishing one batch pushes those
// questions to the back and the next batch surfaces something different —
// until everything's been touched at least once today, at which point
// repeats are expected (you've covered the whole pool for today).
export function filterPriorityMix(entries, limit) {
  ...
  const mistakes = entries
    ...
    .sort((a, b) => reviewedToday(a) - reviewedToday(b) || (b.wrongAttempts || 0) - (a.wrongAttempts || 0));
  const rest = entries
    .filter((e) => !((e.wrongAttempts || 0) > 0))
