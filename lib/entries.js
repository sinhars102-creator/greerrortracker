...
function rowToEntry(row) {
  return {
    ...
    starred: row.starred,
    wrongAttemptsAtStar: row.wrong_attempts_at_star,
    relook: row.relook,
    totalAttempts: row.total_attempts,
    wrongAttempts: row.wrong_attempts,
    lastTimeSpentSeconds: row.last_time_spent_seconds,
    attemptHistory: row.attempt_history || [],
    retestCleared: row.retest_cleared,
    pending: row.pending,
...
function entryToRow(e, userId) {
  ...
  if (e.starred !== undefined) row.starred = e.starred;
  if (e.wrongAttemptsAtStar !== undefined) row.wrong_attempts_at_star = e.wrongAttemptsAtStar;
  if (e.relook !== undefined) row.relook = e.relook;
  ...
  if (e.lastTimeSpentSeconds !== undefined) row.last_time_spent_seconds = e.lastTimeSpentSeconds;
  if (e.attemptHistory !== undefined) row.attempt_history = e.attemptHistory;
  if (e.retestCleared !== undefined) row.retest_cleared = e.retestCleared;
...
// Supabase/PostgREST silently caps an unbounded select() at the project's
// "Max Rows" setting (1000 by default) — past that, older rows just never
// come back, no error, nothing to indicate truncation happened. Once this
// user passed ~1000 entries, every page relying on listEntries() (Review,
// Dashboard, All Entries, ...) was silently missing however many of their
// oldest logged mistakes fell past the cutoff — and since it's ordered
// newest-first, every new entry pushed one more real entry out of view.
// Paginate explicitly with .range() so the full set always comes back
...
// In-memory only (cleared on reload) — signed URLs last an hour, so within
// one session there's no reason to re-request the same path every time a
// question is revisited (Previous/jump-back) or prefetched ahead of time.
// Cached for less than the real 3600s lifetime so a URL is never served
// right at the edge of expiring.
const signedUrlCache = new Map(); // path -> { url, expiresAt }
export async function getScreenshotUrlCached(path) {
